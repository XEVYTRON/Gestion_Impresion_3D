...
if 'df_facturas' not in st.session_state:
    st.session_state.df_facturas = None
...
if st.session_state.seccion == "TRABAJOS":
    ...
    if texto_buscar:
        ...
        try:
            est_sel = st.pills("Estado:", ESTADOS, default="Pendiente")
        except:
    ...
    for idx, r in items_mostrar.iterrows():
        id_job = str(r['ID'])
        nota_limpia = str(r['Notas']).strip()
        badge = r['Estado'] if texto_buscar else ""

        with st.container():
            st.markdown(card_html(r['Fecha'], id_job, r['Cliente'], r['Pieza'], nota_limpia, float(r['Precio']), badge), unsafe_allow_html=True)
            ...
            with st.expander("MODIFICAR ⚙️"):
                with st.form(f"fm_{id_job}"):
                    ...
                    en = st.text_area("Notas", value=nota_limpia)
                ...
                if not st.session_state[ck]:
                    ...
                    st.warning("¿Borrar este trabajo?")
                    b1, b2 = st.columns(2)
    ...
    with st.container(key=f"cn_{st.session_state.form_reset_key}"):
        ...
        if st.button("GUARDAR"):
            if nuevo_cliente and nueva_pieza:
                ...
                row = pd.DataFrame([{"ID": id_n, "Fecha": datetime.now().strftime("%d/%m/%Y"), "Cliente": nuevo_cliente, "Pieza": nueva_pieza, "Estado": "Pendiente", "Precio": pf, "Gramos": gms, "Horas": hrs, "Notas": str(nn).strip()}])
...
# --- 10. FACTURAS ---
    ...
    for i, r in items_f.iterrows():
        nota_h = str(r['Notas']).strip()
        with st.container():
            st.markdown(card_html(r['Fecha'], r['ID'], r['Cliente'], r['Pieza'], nota_h, float(r['Precio'])), unsafe_allow_html=True)
